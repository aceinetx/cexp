...
static void cexp_init_num_stack(NumStack *s) { s->top = -1; }

static void cexp_push_num(NumStack *s, cexp_number value) {
  s->items[++(s->top)] = value;
}

static cexp_number cexp_pop_num(NumStack *s) { return s->items[(s->top)--]; }

static void cexp_init_char_stack(CharStack *s) { s->top = -1; }

static void cexp_push_char(CharStack *s, char value) {
  s->items[++(s->top)] = value;
}

static char cexp_pop_char(CharStack *s) { return s->items[(s->top)--]; }

static char cexp_peek_char(CharStack *s) { return s->items[s->top]; }

static bool cexp_is_empty_char(CharStack *s) { return s->top == (size_t)-1; }

static cexp_bool cexp_is_digit(char c) { return (c >= '0' && c <= '9'); }
static cexp_bool cexp_is_operator(char c) {
  return (c == '+' || c == '-' || c == '*' || c == '/');
}

static cexp_number perform_operation(cexp_number operand1, cexp_number operand2,
                                     char operator) {
  switch (operator) {
  case '+':
...
void cexp_lexer_delete(Cexp_Lexer *lexer) { free(lexer); }
...
static Cexp_Token cexp_lexer_number(Cexp_Lexer *this) {
  ...
  token.value =
      (left_part + (right_part / (cexp_number)pow(10, right_part_length)));
  ...
  while (token.type != CEXP_NONE) {
    if (token.type == CEXP_NUMBER) {
      ...
      while (!cexp_is_empty_char(&operators) &&
             cexp_is_operator(cexp_peek_char(&operators)) &&
             ((token.ch == '+' || token.ch == '-') ||
              ((token.ch == '*' || token.ch == '/') &&
               (cexp_peek_char(&operators) == '*' ||
                cexp_peek_char(&operators) == '/')))) {
        cexp_number operand2, operand1;

        operand2 = cexp_pop_num(&values);
        operand1 = cexp_pop_num(&values);
        cexp_push_num(&values, perform_operation(operand1, operand2,
